...
let print_boolean_list l =
  List.iteri l ~f:(fun idx value -> if value then printf "%d " idx);
  printf "\n"
;;

let print_stabilized_set s =
  Set.iter !s ~f:(fun idx -> printf "%d " idx);
  printf "\n";
  s := Int.Set.empty
;;
...
let%expect_test "select_one" =
  let var, stabilized, o =
    setup_test ~list_size:5 ~init:2 ~f:Fn.id (S.select_one (module Int))
  in
...
let%expect_test "select_one'" =
  let var, stabilized, o =
    setup_test ~list_size:5 ~init:None ~f:Fn.id (S.select_one' (module Int))
  in
  let print_list () = print_boolean_list (Incr.Observer.value_exn o) in
  Incr.stabilize ();
  print_stabilized_set stabilized;
...
let%expect_test "select_one_value" =
  ...
  in
  let print_list () =
    List.iter (Incr.Observer.value_exn data) ~f:(fun value -> printf "%d " value);
    printf "\n"
  in
  Incr.stabilize ();
  print_stabilized_set stabilized;
...
let%expect_test "select_many" =
  ...
  print_stabilized_set stabilized;
  [%expect {| 2 3 7 8 9 |}];
  (* cutoff because only 10 watchers *)
  print_list ();
  [%expect {| 7 8 9 |}]
;;
...
let%expect_test "select_many_values" =
  ...
  in
  let print_list () =
    List.iter (Incr.Observer.value_exn data) ~f:(fun data -> printf "%d " data);
    printf "\n"
  in
  Incr.stabilize ();
  print_stabilized_set stabilized;
